#!/usr/bin/env python3
"""
Coding Agent 4 - Multi-Agent Development System
...
class CodingAgent:
    def __init__(self, agent_id="coder4"):
        self.agent_id = agent_id
        
        # Multi-API configuration for coding (RELIABLE MODELS - different priority for coder4)
        self.apis = {
            "cohere": {
        ...
        # Current API selection (start with Cohere for diversity)
        self.current_api = "cohere"
...
Create complete, functional code that fits into the {project_info['project_type']} project architecture."""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Adaptive token management based on API capabilities
                max_tokens = 8000
                if self.current_api == "cohere":
                    max_tokens = 4000  # Cohere limit
                elif self.current_api == "openrouter" and "free" in current_model:
                    max_tokens = 6000  # Conservative for free models
                
                code_content = self.make_api_request(system_prompt, user_prompt, max_tokens)
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python coder4.py <task_file_path>")
        sys.exit(1)
    
    task_file_path = sys.argv[1]
    
    try:
        agent = CodingAgent("coder4")
